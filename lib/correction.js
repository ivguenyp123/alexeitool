...
/**
 * ── LE MODÈLE NE DONNE PAS TOUJOURS LE MOT CORRIGÉ ──────────────────────────
 *
 * Mesuré sur la deuxième vraie dictée. Au lieu de « parte → partent », il a écrit :
 *
 *     2. « parte » → « accord sujet/verbe (3e personne du pluriel). »
 *
 * Il NOMME la faute, il ne la corrige pas. Et comme la nature était entre guillemets, je
 * la prenais pour le mot correct : la copie se retrouvait avec douze « signalé mais
 * introuvable », et pas une marque rouge.
 *
 * Une nature n'est pas une correction. On la reconnaît à son vocabulaire — accord,
 * orthographe, conjugaison — et à sa longueur : un mot corrigé, c'est un mot, pas une
 * phrase. Ce qui n'en est pas une devient un SIGNALEMENT : le mot est souligné en rouge
 * et la nature écrite à côté, ce qui est exactement le geste d'un enseignant qui laisse
 * l'élève trouver.
 */
const VOCABULAIRE = /\b(accord|erreur|faute|orthographe|conjugaison|homophone|lexicale?|pluriel|singulier|personne|démonstratif|possessif|participe|majuscule|ponctuation|invariable|terminaison|désinence|grammaticale?|segmentation)\b/i;

export function estUneCorrection(mot) {
  const m = String(mot || '').trim();
  if (!m) return false;
  const mots = m.split(/\s+/);
  // Un mot corrigé est court. Passé cinq mots ou quarante-cinq signes, c'est une phrase.
  if (mots.length > 5 || m.length > 45) return false;
  // Un point au milieu, c'est une phrase qui se termine.
  if (/[.;]\s+\S/.test(m)) return false;
  return !(VOCABULAIRE.test(m) && mots.length > 1);
}
...
const nettoie = (s) => String(s || '')
  .replace(/\*\*/g, '')
  .replace(/^[\s*·—–-]+/, '')
  // La numérotation d'une liste : « 1. », « 12) ». Sans ça, le mot fautif relevé était
  // « 1. « Se matin » — introuvable dans la copie, forcément.
  .replace(/^\d{1,3}\s*[.)]\s*/, '')
  .replace(/^[\s*·—–-]+/, '')
  .replace(/\s+$/, '');
...
export function lireUneErreur(ligne) {
  ...
  /*
   * Pas de correction, seulement une nature : c'est un SIGNALEMENT. On ne fabrique pas le
   * mot manquant — on souligne celui qui est fautif et on écrit la nature à côté.
   */
  if (!propre || !estUneCorrection(propre)) {
    const dit = nettoieNature([propre, nature].filter(Boolean).join(' '));
    /*
     * Une nature tient en deux ou trois mots — « accord sujet-verbe », « homophone a/à ».
     * Au-delà, ce n'est plus une nature : c'est une phrase, donc une ligne de prose qui
     * contenait une flèche par hasard. On la rend au texte plutôt que d'en faire un
     * signalement sur un mot qui n'en est pas un.
     */
    if (!dit || dit.length > 60 || dit.split(/\s+/).length > 8) return null;
    return { ecrit, attendu: '', nature: dit };
  }
...
export default { lireLaCorrection, lireUneErreur, estUneCorrection, affiner, apparier };
