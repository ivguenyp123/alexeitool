...
export function marquer(texte, erreurs = []) {
  ...
  for (const t of trouvees) {
    ...
    /*
     * ── DEUX GESTES, SELON CE QU'ON SAIT ───────────────────────────────────
     *
     * Le mot corrigé est connu : on barre le fautif et on écrit le bon à côté.
     *
     * Il ne l'est pas — le modèle a seulement nommé la faute : on SOULIGNE le mot et on
     * écrit la nature. C'est exactement ce que fait un enseignant qui veut que l'élève
     * trouve lui-même, et c'est déjà l'essentiel : voir les fautes tout de suite.
     *
     * Ce qu'on ne fait dans aucun des deux cas, c'est inventer la correction manquante.
     */
    if (t.attendu) {
       ...
       * mangeait le caractère suivant — « le tapi tapis ." ».
       */
      morceaux.push({ texte: ` ${t.attendu}`, rouge: true, gras: true });
    } else {
      morceaux.push({ texte: t.tel, rouge: true, souligne: true });
      if (t.nature) morceaux.push({ texte: ` (${t.nature})`, rouge: true, taille: 18 });
    }
    curseur = t.fin;
  }
