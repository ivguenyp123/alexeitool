...
export function contexteCreneau(c, { classe = [], attendus = [],
  ...
  L.push('  Ce qui a été fait aux séances précédentes. Tu fais quand même la séance ; si');
  L.push('  elle en dépend, tu mets UNE ligne à la fin sous « À COMPLÉTER ». Pas un');
  L.push('  préambule : personne ne peut te répondre, et un avertissement en tête retarde');
  L.push('  le seul contenu utile.');
  return L.join('\n');
}
...
export function contexteSemaine(semaine = [], { classe = [], annee = anneeScolaire() } = {}) {
  ...
  L.push('  les APC. Si ta proposition suppose un de ces temps-là, mets-le en UNE ligne à');
  L.push('  la fin sous « À COMPLÉTER ».');
  return L.join('\n');
}
...
export function textePile(geste, p, t, { attendus = [], annee = anneeScolaire(),
  ...
  L.push('  a été effacé, la peine que ça a coûté. Tu lis une transcription : tu ne dis donc');
  L.push('  rien de la présentation ni du geste d\'écriture.');
