...
export function caviarderLaPile(p, t) {
  ...
  // La consigne de l'exercice aussi : elle est écrite par l'enseignant, et un énoncé
  // nomme volontiers des enfants de la classe pour les mettre en scène.
  const consigneDonnee = caviarder(p?.consigneDonnee || '', t).texte;

  return { copies, consigneDonnee, combien, remplaces,
...
export function direLaPile(p, t, { attendus = [], annee } = {}) {
  ...
  for (const c of cav.copies) {
    L.push('');
    L.push(`  ── ${c.pseudo || 'Copie non attribuée'}${c.pseudo ? '' : ` (${c.nom || 'sans nom'})`}`);
