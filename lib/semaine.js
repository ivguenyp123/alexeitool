...
export function volumes(creneaux = []) {
  ...
  for (const c of creneaux) {
    ...
    for (const n of NIVEAUX) {
      const domaine = c.regime === 'dedouble' ? c[n]?.domaine : (c.domaine || c[n]?.domaine);
      if (!domaine) continue;
      // En dédoublé, chaque groupe n'a que la moitié du créneau : on mène les deux
      // séances l'une après l'autre. Ne pas diviser gonflerait la semaine d'heures qui
      // n'existent pas.
